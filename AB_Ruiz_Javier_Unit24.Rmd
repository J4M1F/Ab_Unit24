...
```{r}
# Definir las traducciones de las categorías de peso en el orden exacto que aparece en la leyenda
weight_translations <- c(
...
# Actualizar los datos con las categorías en español
weight_analysis_es <- weight_analysis %>%
  mutate(
    weight_class_es = weight_translations[as.character(weight_class)]
  )

# Reordenar los niveles exactamente como en la imagen
weight_analysis_es$weight_class_es <- factor(
  weight_analysis_es$weight_class_es,
  levels = c(
...
# Definir la misma paleta de colores que en la imagen
weight_colors <- c(
  "Peso Mosca" = "#FFB3B3",
  "Peso Gallo" = "#99CCFF",
  "Peso Pluma" = "#B3FFB3",
...
# Crear el gráfico exactamente igual al mostrado
ggplot(weight_analysis_es, 
       aes(x = factor(cluster), 
           y = pct_cluster, 
           fill = weight_class_es)) +
...
```
### Análisis Distribución de Clusters

1.Respecto a la distribución general de agrupaciones:


Los grupos más numerosos son el 1 (378 competidores, 23.9%) y el 4 (359 competidores, 22.7%), constituyendo casi la mitad de los competidores.
El cluster 6 es notablemente el más pequeño (103 peleadores, 6.5%), sugiriendo un grupo más especializado.
Los clusters 2, 3 y 5 mantienen tamaños similares (entre 213-265 luchadores, 13-17%).

2.Características distintivas por agrupación:


Cluster 1 (23.9%): Dominado por pesos ligeros, especialmente peso mosca (41.0%) y peso gallo (25.4%). Clara concentración en categorías ligeras.
Cluster 2 (16.6%): Distribución más variada, con predominio de peso medio (24.8%) y peso semipesado (24.8%), seguido de peso pluma (20.2%).
Cluster 3 (13.5%): Muestra un patrón equilibrado, destacando peso ligero (20.7%) y peso pesado (16.0%).
Cluster 4 (22.7%): Notable presencia de peso medio (26.7%) y peso pesado (25.1%).
Cluster 5 (16.8%): Destaca por peso ligero (22.6%) y peso semipesado (23.0%).
Cluster 6 (6.5%): El más pequeño, con predominio de peso pluma (17.5%) y peso ligero (16.5%).

3.Patrones llamativos:


Existe una clara diferenciación en la distribución de pesos entre clusters.
Algunos clusters muestran preferencia por categorías específicas mientras otros son más diversos.
Los pesos extremos (mosca y pesado) tienden a concentrarse en clusters específicos.

4.Implicaciones:

La distribución sugiere que los estilos de lucha pueden transcender las categorías de peso.
Cada cluster parece representar un perfil específico de luchador, combinando peso y probablemente estilo de pelea.
La variación en el tamaño de los clusters indica que algunos estilos o combinaciones de características son más comunes que otros en el deporte.
La existencia de clusters tan definidos sugiere patrones naturales en cómo se agrupan las características de los luchadores.
...
print(density_plot)
```
#### Distribución de Métricas por Cluster (Gráfico de Densidad):


Derribos por Combate: Mayor concentración entre 0-2.5 derribos, con el Cluster 6 mostrando una distribución más extendida hacia valores altos.
Eficiencia en Derribos: Distribución concentrada en valores bajos, con algunos clusters mostrando picos en el rango 0.25-0.50.
Eficiencia en Striking: Alta densidad en valores bajos (0-20), con colas largas hacia valores más altos.
Precisión en Derribos: Distribución multimodal, con varios clusters mostrando picos en diferentes rangos.
Precisión en Striking: Distribución más uniforme entre clusters, centrada aproximadamente en 0.40-0.60.
...
```
#### Características de Grappling por Cluster:


Derribos Promedio: Destacable variación entre clusters:

Cluster 1: ~1.75 derribos
Cluster 6: >4 derribos por combate, significativamente superior
Resto de clusters: valores entre 0.5-1.5 derribos


Eficiencia en Derribos: Cluster 6 muestra la mayor eficiencia, seguido por Cluster 1
Intentos de Sumisión: Clusters 3 y 6 destacan con >1 intento por combate
Precisión en Derribos: Valores superiores en Clusters 1 (~40%) y 6 (~50%)
...
#### Características de Striking por Cluster:


Eficiencia Striking:

Cluster 5: Destaca notablemente (~20)
Resto de clusters: Valores entre 2-5


Golpes Significativos:

Cluster 5: Aproximadamente 40 golpes
Otros clusters: Valores relativamente uniformes entre 5-10


Precisión Striking:

Tendencia ascendente del Cluster 1 al 6
Cluster 6: Mayor precisión (~52%)
Valores generales entre 40-52%
...
Cluster 5: Valor extraordinariamente alto (995.5%)
Cluster 6: Segundo más alto (340.9%)
Resto: Valores entre 170-260%


Defensiva:

Valores más uniformes entre clusters (45-70%)
Menor variabilidad que en la ofensiva
...
El Cluster 6 sobresale en grappling, fusionando una elevada eficacia en derribos con elevados números en striking.
El Cluster 5 exhibe una supremacía excepcional en la ofensiva de striking.
Los Clusters 1-4 exhiben perfiles más equilibrados, con especializaciones de moderada intensidad.

- Patrones de Efectividad:


Existe una correlación positiva entre la precisión y la eficiencia en ambos aspectos (striking y grappling)
Los clusters más especializados (5 y 6) muestran mayor efectividad en sus áreas respectivas
La precisión en striking muestra una progresión positiva a través de los clusters

- Implicaciones Estratégicas:


Se identifican dos enfoques de alta especialización: striking dominante (Cluster 5) y grappling avanzado con striking preciso (Cluster 6)
Los clusters intermedios sugieren diferentes balances entre striking y grappling
La variabilidad en el balance ofensivo-defensivo indica distintas aproximaciones al combate
